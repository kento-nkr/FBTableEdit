class FBTableModal {
  ...
  #showPageContent(modalBody, rowNum) {
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      pageButton.setAttribute(
        "class",
        "btn btn-outline-secondary page-switch-button"
      );
    ...
    targetTableContent.forEach((input) => {
      ...
      if (
        this.listener_obj != undefined &&
        this.listener_obj[input.id] != undefined
      ) {
        //listener_obj内で指定されている場合、EventListenerの設定を行う
        const keys = Object.keys(this.listener_obj[input.id]);
        keys.forEach((key) => {
          inputElement.addEventListener(key, () =>
            this.listener_obj[input.id][key](this)
          );
        });
      }
  ...
  setDatalist(
    id,
    valueArray //idにはtable_fieldcodeが勝手に前方付与される
  ) {
    // 引数の形式が正しいことを確認
    if (
      ...
      console.error(
        "setDatalist関数の引数に誤りがあります。(id:str, valueArray:Arrat)"
      );
      return;
    }
  ...
  addEditButton() {
    ...
    button.addEventListener("click", () => {
      ...
      $("#" + this.modalId).on("hidden.bs.modal", () => {
        const modalRowNumAndValue = this.getValuesFromModal();
        this.table_values[modalRowNumAndValue.rowNum] =
